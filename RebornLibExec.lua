...
function Window:ShowBoot()
	if not self._bootFrame or not self._mainFrame then return end
...
function Window:CreateTab(config)
	...
	-- comment if you dont want gradient, uncomment if you do
	...
	]] -- comment if you dont want gradient, uncomment if you dont
...
function Section:CreateSlider(config)
	...
	local function snap(v)
		return math.clamp(
			math.floor((v - min) / step + 0.5) * step + min,
			min,
			max
		)
	end
