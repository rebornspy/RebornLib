...
UserInputService.InputBegan:Connect(function(input, gp)
  if gp or Window._isBindingKey then return end
...
local function setupCharacter(char)
  ...
  RunService.RenderStepped:Connect(function()
    ...
    -- Noclip
    if head then head.CanCollide = not noclipEnabled end
    if torso then torso.CanCollide = not noclipEnabled end
  end)
end
--[[
-- Mouse TP
...
UserInputService.InputBegan:Connect(function(input, gp)
  if gp or not infiniteJumpEnabled then return end
...
combatSection:CreateToggle({
  ...
  Callback = function(state)
    autoDie = state
    local hum = player.Character and player.Character:FindFirstChildOfClass("Humanoid")
    if state and hum then hum.Health = 0 end
  end,
})
...
local function grabUpdate()
  ...
  workspace.GrabParts.DragPart.AlignPosition.Position = char.Head.Position + (workspace.CurrentCamera.CFrame.LookVector.Unit * dist)
end
...
local function grabStartLoop()
  if grabLoopRunning then return end
...
combatSection:CreateInput({
  ...
  Callback = function(text)
    ...
    if num and num > 0 then scrollIncrement = num end
  end
})
...
player.CharacterAdded:Connect(function(newChar)
  ...
  if grabEnabled then grabApply() end
end)
...
local function grabKillStart()
  if grabKillConnection then grabKillConnection:Disconnect() end

  grabKillConnection = workspace.ChildAdded:Connect(function()
    if not grabKillEnabled then return end
    ...
    if ignoreNames[grabbed.Name] then return end
...
combatSection:CreateToggle({
  ...
  Callback = function(state)
    ...
    if state then grabKillStart() else grabKillStop() end
  end
})
...
local function startOPKG()
  if OPKGConnections then OPKGConnections:Disconnect() end

  OPKGConnections = workspace.ChildAdded:Connect(function()
    if not OPKillGrabEnabled then return end

    local grabparts = workspace:FindFirstChild("GrabParts")
    if not grabparts or not grabparts:IsA("Model") then return end
    ...
    local grabbed = weld.Part1.Parent
    if ignoreNames[grabbed.Name] then return end
    if not grabbed:IsA("Model") then return end
    ...
    if humanoid then
      ...
      grabbed:PivotTo(
        grabbed:PivotTo(grabbed:GetPivot():Lerp(target, 0.01))
      )
    else
      warn("No Humanoid!")
    end
...
combatSection:CreateToggle({
  ...
  Callback = function(state)
    ...
    if state then startOPKG() else stopOPKG() end
  end
})
...
local function startAntiGrab()
  ...
  antiGrabConnection = workspace.DescendantAdded:Connect(function(desc)
    ...
    if desc.Name ~= "GrabParts" then return end
...
testingSection1:CreateDropdown({
  Name = "Dropdown Test",
  Options = {
    "TwentyOneCharactersss", "Button2", "Button3", "Button4", "Button5",
